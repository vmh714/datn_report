...
\label{section:2.2}
Phần này mô tả tổng quan các chức năng cốt lõi của hệ thống, dựa trên định hướng phân chia hai vai trò người dùng chính: Quản trị viên (Admin) và Nhân viên y tế (Medical Staff), cùng với tác nhân hệ thống là Thiết bị giám sát (Edge Node).

\subsection{Biểu đồ use case tổng quát}
\label{subsection:2.2.1}
Biểu đồ use case tổng quát mô tả các tương tác chính giữa người dùng và hệ thống. Các tác nhân (Actors) tham gia bao gồm:
\begin{itemize}
    \item \textbf{Quản trị viên (Admin):} Đảm nhiệm vai trò thiết lập hệ thống, quản lý cơ sở dữ liệu thiết bị và cấp quyền cho nhân viên y tế.
    \item \textbf{Nhân viên y tế (Medical Staff):} Người trực tiếp sử dụng hệ thống để giám sát, theo dõi hồ sơ bệnh nhân, quản lý việc cấp phát thiết bị và xử lý các sự cố té ngã.
    \item \textbf{Thiết bị giám sát (ESP32 Node):} Tác nhân hệ thống (System Actor), liên tục gửi dữ liệu viễn trắc (telemetry) thông qua giao thức PPPoS trên mạng 4G LTE và kích hoạt các cảnh báo (alerts) khẩn cấp.
\end{itemize}

Các ca sử dụng (Use case) cốt lõi bao gồm: Đăng nhập \& Xác thực, Quản lý thiết bị, Quản lý nhân sự, Quản lý bệnh nhân (Wearer), Gán/Thu hồi thiết bị, Giám sát thời gian thực, Xử lý cảnh báo té ngã, và Xem thống kê vận động.

\subsection{Biểu đồ use case phân rã "Giám sát thời gian thực"}
\label{subsection:2.2.2}
Use case "Giám sát thời gian thực" yêu cầu hệ thống cung cấp cho nhân viên y tế một giao diện Dashboard tổng quan. Thông qua giao diện này, nhân viên y tế có thể theo dõi danh sách toàn bộ các thiết bị đang hoạt động, xem trạng thái kết nối mạng, mức pin hiện tại của từng thiết bị. Đặc biệt, chức năng này sẽ liên tục lắng nghe các gói tin từ MQTT Broker để ngay lập tức hiển thị cảnh báo (kèm âm thanh và viền đỏ nhấp nháy) khi có sự cố té ngã xảy ra. Biểu đồ phân rã cho nhóm chức năng này tập trung vào luồng hiển thị dữ liệu và cơ chế cảnh báo.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{4cm}|p{10.5cm}|}
        ...
        \textbf{Tác nhân} & Nhân viên y tế, Quản trị viên \\ \hline
...
\begin{itemize}
    ...
    \item \textbf{Khả năng mở rộng (Scalability):} Kiến trúc Dual-Database giúp hệ thống xử lý đồng thời hàng nghìn luồng dữ liệu viễn trắc (InfluxDB) mà không gây tắc nghẽn cho các truy vấn quản lý (PostgreSQL).
    \item \textbf{Tính bảo mật (Security):} Mọi API giao tiếp từ Frontend đều phải được xác thực thông qua JSON Web Token (JWT), chống rò rỉ thông tin cá nhân của bệnh nhân.
\end{itemize}
